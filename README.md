...
> **On-disk cache: security model (read before enabling).** Persisting resolved secrets to disk with `FileWriterLayer` is an explicit, opt-in tradeoff. The cache file is **scrambled, not encrypted**: obfuscation that defeats `cat`/`grep`, secret scanners, and backups, and makes a copy taken off the machine useless (the key is derived from machine-local material). It does **not** protect against a process running as your own user - anyone with your uid on the box can recover the values. The risk is bounded by design: entries live only for their TTL, the file defaults to a RAM-backed location that clears on logout, and it is `0600` inside a `0700` directory, ignored on load if its ownership or permissions look tampered with. If that tradeoff is not acceptable for your threat model, use `MemoryLayer` (process-only) or no on-disk cache at all.
...
`v0.9.0` — pre-1.0. The public API is stable enough to build against; minor breaking changes are possible before `v1.0`. Track [`CHANGELOG.md`](CHANGELOG.md) for what changes between releases.
