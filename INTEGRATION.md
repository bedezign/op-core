...
`v0.7.1` — pre-1.0. The API is stable enough to build against, but minor breaking changes are possible before `v1.0`. Specifically:
...
```bash
uv add "op-core @ git+https://github.com/bedezign/op-core@v0.7.1"
pip install "op-core @ git+https://github.com/bedezign/op-core@v0.7.1"
```
...
[project]
dependencies = [
    "op-core @ git+https://github.com/bedezign/op-core@v0.7.1",
]
```
...
[tool.uv.sources]
op-core = { git = "https://github.com/bedezign/op-core", tag = "v0.7.1" }
```
...
Persisting resolved secrets to disk with `FileWriterLayer` (or `op-env --ttl`) is an explicit, opt-in tradeoff - enable it deliberately.

The file is **scrambled, not encrypted.** The serialized payload is zlib-compressed and XOR-ed with a SHA-256 keystream derived from machine-local material (machine id + uid) plus a per-write random nonce. This is obfuscation, not cryptography: it defeats casual or offline reading - `cat`/`grep`, secret scanners, backups, or the file copied off the machine (where the key material is absent) - but it does **not** protect against a process running as your own user. Anyone with your uid on the machine can recover the values.

The risk is bounded by design:

- **Temporary.** Every entry expires against its writer's TTL; nothing is kept indefinitely.
- **RAM-backed by default.** The default location is under `$XDG_RUNTIME_DIR`, which the system clears on logout.
- **Locked down.** The file is `0600` inside a `0700` directory, owned by you, and is ignored on load if its ownership or permissions look tampered with (guarding the classic shared-`/tmp` pre-creation attack).
- **Fail-safe.** A corrupt or untrusted cache never crashes the caller; it degrades to empty.

If this tradeoff is not acceptable for your threat model, use `MemoryLayer` (process-only) or no cache at all. The full threat model is documented in the `op_core.backends.file_caching` module docstring.
